...
public:
  ...
  virtual Value getMPIOp(const Location loc,
                         ConversionPatternRewriter &rewriter,
                         mlir::mpi::MPI_ReductionOpEnum opAttr) = 0;
};
...
struct BarrierOpLowering : public ConvertOpToLLVMPattern<mpi::Barrier> {
  using ConvertOpToLLVMPattern::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(mpi::Barrier op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    // get some helper vars
    Location loc = op.getLoc();
    MLIRContext *context = rewriter.getContext();
    ...
    auto rankFuncType =
        LLVM::LLVMFunctionType::get(i32, {comm.getType()});
    // get or create function declaration:
    LLVM::LLVMFuncOp initDecl = getOrDefineFunction(
        moduleOp, loc, rewriter, "MPI_Barrier", rankFuncType);

    // replace with function call
    auto callOp = rewriter.create<LLVM::CallOp>(
        loc, initDecl, ValueRange{comm});
